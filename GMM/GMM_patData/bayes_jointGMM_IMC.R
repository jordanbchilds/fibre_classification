...
mcmc_plots = function(mcmc_output, title){
  op = par(mfrow=c(1,2), mar = c(5.5,5.5,3,3))
  plot(mcmc_output)
  autocorr(mcmc_output)
  title(main=title, line = -1, outer = TRUE)
  par(op)
...
dir.create(file.path("PDF/PDF_joint/MCMC_output"), showWarnings = FALSE)
dir.create(file.path("PDF/PDF_joint/classification"), showWarnings = FALSE)
...
for( chan in imc_chan[-which(imc_chan == 'VDAC1')]){
  for( pat in pts){
    ...
    if( !file.exists(posterior_file)){
      ...
      pdf(file.path("PDF/PDF_joint/classification",paste0(outroot,".pdf")),width=14,height=8.5)
      
      priorpost_pred(Yctrl=XY_ctrl, Ypat=XY_pat, posterior=posterior, prior=prior, 
                     classifs=classifs, title=paste(froot, pat)  )
      
      pdf(file.path("PDF/PDF_joint/MCMC_output",paste0(outroot,".pdf")),width=14,height=8.5)
      
      mcmc_plots(mcmc_output = MCMCoutput, title = paste(froot, pat))
