...
mvdnorm = function(x, mu, Sigma){
  density = rep(NA, nrow(x))
  for(i in 1:nrow(x)){
    density[i] = 1/det(Sigma)^(0.5) * exp( -0.5*t(x[i,]-mu)%*%solve(Sigma)%*%(x[i,]-mu))
  }
  return(density)
}

plot_comps = function(ctrl_data, pat_data, class, X1, X2, title){
  par(mfrow=c(1,2))
  den1 = percentiles(X1[,1], X1[,2])
  contour(den1$dens, levels=den1$levels, labels=den1$probs, 
          col="blue", lwd=2,main="Component One", xlim=c(-1,5), ylim=c(-1,5),
          xlab=paste0("log(",mitochan,")"), ylab=paste0("log(",chan,")"), 
          cex.lab=1.4, cex.axis=1.4, cex.main=1.7)
  points(ctrl_data[,1], ctrl_data[,2], col=myDarkGrey, pch=20)
  points(pat_data[,1], pat_data[,2], col=classcols(class), pch=20)
  
  den2 = percentiles(X2[,1], X2[,2])
  contour(den2$dens, levels=den2$levels, labels=den2$probs, 
          col="red", lwd=2,main="Component Two", xlim=c(-1,5), ylim=c(-1,5),
          xlab=paste0("log(",mitochan,")"), ylab=paste0("log(",chan,")"), 
          cex.lab=1.4, cex.axis=1.4, cex.main=1.7)
  points(ctrl_data[,1], ctrl_data[,2], col=myDarkGrey, pch=20)
  points(pat_data[,1], pat_data[,2], col=classcols(class), pch=20)
  
  title(main=title, outer=T, line=-1)
}

plot_prob = function(classifs, title){
  plot( density(classifs), lwd=2, col="blue", xlim=c(0,1),
        main="prodiff ish", ylab="", xlab="")
  title(main=title, outer=T, line=-1)
}
...
chan = "MTCO1"
pts = c("P01", "P02", "P03", "P04", "P05", "P07", "P08", "P09", "P10")
...
posteriors = function(data){
  ...
  mcmc.filepath = file.path("Output/IMC_joint2", paste(chan, pat, "POSTERIOR.txt", sep="__") )
  mcmc.output = read.delim(file=mcmc.filepath, header=TRUE, sep=" ", stringsAsFactors=FALSE)
  
  m1 = colMeans(mcmc.output[,c("mu.1.1.","mu.2.1.")])
  T1 = matrix( colMeans(mcmc.output[,c("tau.1.1.1.","tau.1.2.1.","tau.2.1.1.","tau.2.2.1.")]),
               ncol=2, nrow=2)
  m2 = colMeans(mcmc.output[,c("mu.1.2.","mu.2.2.")])
  T2 = matrix( colMeans(mcmc.output[,c("tau.1.1.2.","tau.1.2.2.","tau.2.1.2.","tau.2.2.2.")]),
               ncol=2, nrow=2)
  
  X1 = mvrnorm(n=10000, mu=m1, Sigma=solve(T1))
  X2 = mvrnorm(n=10000, mu=m2, Sigma=solve(T2))
  
  f0 = mvdnorm(XY_pat, mu=m1, Sigma=solve(T1))
  f1 = mvdnorm(XY_pat, mu=m2, Sigma=solve(T2))
  
  classifs = f0/(f0+f1)
  
  return(list(
    "plot_comps" = function(){
      plot_comps(ctrl_data=XY_ctrl, pat_data=XY_pat, class=classifs, X1=X1, X2=X2, title=paste(chan, pat, sep=" "))
    },
    "plot_prob" = function(){
      plot_prob(classifs, title=paste(chan, pat, sep=" "))
    }
  ))
}
...
for(chan in imc_chan){
  for(pat in pts){
    name = paste(chan, pat, sep="_")
    chanpat[[name]] = list(chan=chan, pat=pat)
  }
}

make_plots = lapply(chanpat, posteriors)

pdf("PDF/comps.pdf", width=14, height=8.5)
for(chan.pat in names(chanpat)){
  make_plots[[chan.pat]][["plot_comps"]]()
}
dev.off()

pdf("PDF/probiffs.pdf", width=7, height=8.5)
for(chan.pat in names(chanpat)){
  make_plots[[chan.pat]][["plot_prob"]]()
}
dev.off()
