...
priorpost_den = function(ctrl_data, ctrl_prior, ctrl_posterior, 
  ...
  if( is.null(pat_data) ){ # plot ctrl prior and posterior
    ...
    plot( pat_posterior[,'Y_syn[1]'], pat_posterior[,'Y_syn[2]'], col=myDarkGrey, pch=20, cex.lab=2, cex.axis=1.5,
          xlab=paste0("log(",mitochan,")"), ylab=paste0("log(",chan,")"), main='Patient Posterior')
    contour( kde2d(pat_posterior[,'Y_syn[1]'], pat_posterior[,'Y_syn[2]'], n=100), add=TRUE, nlevels=5)
...
dir.create(file.path("Output/Output_deltaIMC"), showWarnings = FALSE)
dir.create(file.path("PDF/PDF_deltaIMC"), showWarnings = FALSE)
dir.create(file.path("PNG/PNG_deltaIMC"), showWarnings = FALSE)
...
for( chan in c('NDUFB8', 'GRIM19')){ # delta test
  
  outroot_ctrl = paste( froot, 'CTRL', chan, sep='__')
  posterior_ctrl_file = file.path("Output/Output_deltaIMC",paste0(outroot_ctrl,"__POSTERIOR.txt"))
  ...
  if(!file.exists(posterior_ctrl_file)){
    ...
    pdf(file.path("PDF/PDF_deltaIMC",paste0(ctrlroot,".pdf")),width=14,height=7)
    priorpost_den( ctrl_prior=prior_ctrl, ctrl_posterior=posterior_ctrl, ctrl_data=data_ctrl, 
  ...
  for(pat in pts){ # loop through patients
    ...
    posterior_file = file.path("Output/Output_deltaIMC",paste0(outroot,"__POSTERIOR.txt"))
    ...
    if(!file.exists(posterior_file)){ # regression for mitochondrial disease patients
      ...
      pdf(file.path("PDF/PDF_deltaIMC",paste0(outroot,".pdf")),width=14,height=8.5)
      priorpost_den(ctrl_data=data_ctrl, ctrl_prior=prior_ctrl, ctrl_posterior=posterior_ctrl, 
                ...
                pat_data=data_pat, classifs=classifs_pat, title=paste(delta_vec,froot,pat) )
      dev.off()
      
      write.table(as.numeric(classifs_pat),file.path("Output/Output_deltaIMC",paste0(outroot,"__CLASS.txt")),row.names=FALSE,quote=FALSE,col.names=FALSE)
      ...
      class_pat_file = file.path("Output/Output_deltaIMC", paste0(outroot, "__CLASS.txt"))
    }
  }
}
