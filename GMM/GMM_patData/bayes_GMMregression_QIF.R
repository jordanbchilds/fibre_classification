...
priorpost = function(prior, posterior, data, classifs, ctrl=FALSE){
  ...
  z_index_post = rep(TRUE, ncol(posterior))
  z_index_prior = rep(TRUE, ncol(prior))
  param_split_post = strsplit(colnames(posterior), split='')
  param_split_prior = strsplit(colnames(prior), split='')
  for(i in 1:length(z_index_prior)){
    if( 'z' %notin% param_split_prior[[i]]) z_index_prior[i] = FALSE
  }
  for(i in 1:length(z_index_post)){
    if( 'z' %notin% param_split_post[[i]]) z_index_post[i] = FALSE
  }
  class_posterior = colMeans(posterior[,z_index_post])
  class_prior = colMeans(prior[,z_index_prior])
  ...
  contour( kde2d(prior[,'Y_syn[1]'], prior[,'Y_syn[2]'], n=100), add=TRUE, nlevels=20)
  
  plot( data$Y[,1], data$Y[,2], col=classcols(class_posterior), pch=20, cex.lab=2,cex.axis=1.5,
        xlab=paste0("log(",mitochan,")"),ylab=paste0("log(",chan,")"))
  contour( kde2d(posterior[,'Y_syn[1]'], posterior[,'Y_syn[2]'], n=100), add=TRUE, nlevels=15)
  ...
  contour( kde2d(prior[,'mu[1,1]'], prior[,'mu[1,2]'], n=100), cex.lab=2, cex.axis=1.5,
        xlab=expression(mu[11]), ylab=expression(mu[12]), 
  ...
  contour( kde2d(prior[,'mu[2,1]'], prior[,'mu[2,2]'], n=100), cex.lab=2, cex.axis=1.5,
        xlab=expression(mu[21]), ylab=expression(mu[22]), 
...
for(fulldat in c("IMV.E02.P01.PMT.M3243AG.QIF.TGo.RAW.txt")){
  ...
  for(chan in c("NDUFB8")){ # only run inference for NDUFB8
    ...
    if(!file.exists(posterior_ctrl_file)){
      ...
      pdf(file.path("PDF",paste0(ctrlroot,".pdf")), width=14, height=7)
      priorpost(prior=prior_ctrl, posterior=posterior_ctrl, 
                data=data_ctrl, classifs=classifs_ctrl)
      title(paste(froot,"CTRL"), line = -1, outer = TRUE)
      dev.off()
    ...
    for(pat in pts){ # loop through patients
      ...
      if(!file.exists(posterior_file)){ # regression for mitochondrial disease patients
        ...
        # posterior
        prec_pred = matrix(0, nrow=2, ncol=2) 
        prec_pred[1,1] = mean(posterior_ctrl[,'tau[1,1,1]'])
        prec_pred[1,2] = mean(posterior_ctrl[,'tau[1,2,1]'])
        prec_pred[2,1] = prec_pred[1,2]
        prec_pred[2,2] = mean(posterior_ctrl[,'tau[2,2,1]'])
        ...
        n_2 = 2
        U_2 = diag(2) #matrix(c(1,0,0,1), ncol=2, nrow=2, byrow=TRUE)
        
        mu1_mean= double(0)
        mu1_mean[1] = mean(posterior_ctrl[,'mu[1,1]'])
        mu1_mean[2] = mean(posterior_ctrl[,'mu[2,1]'])
        ...
        # Bayesian inference using JAGS                                                                                                                                                  
        # Assume a prior centred around the estimate from control data
        Npat = nrow(XY_pat)
        ...
        model_pat=jags.model(textConnection(modelstring),data=data_pat) 
        model_pat_priorpred=jags.model(textConnection(modelstring),data=data_pat_priorpred) 
        update(model_pat,n.iter=MCMCUpdates)
        converge_pat = coda.samples(model=model_pat,variable.names=c("mu","tau","Y_syn","z","probdiff"),n.iter=MCMCUpdates_Report,thin=MCMCUpdates_Thin)
        output_pat = coda.samples(model=model_pat,variable.names=c("mu", "tau","Y_syn","z","probdiff"),n.iter=MCMCUpdates_Report,thin=MCMCUpdates_Thin)
        output_pat_priorpred = coda.samples(model=model_pat_priorpred,variable.names=c("mu", "tau","Y_syn","z","probdiff"),n.iter=MCMCUpdates_Report,thin=MCMCUpdates_Thin)
        ...
        priorpost(prior_pat, posterior_pat, data_pat, classifs_pat)
