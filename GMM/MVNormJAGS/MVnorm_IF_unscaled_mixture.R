...
y = myData[myData$sno=="P04",c("raw_porin","raw_CI","raw_CIV")]
...
cramp = colorRamp(c("red","white","blue"),space="Lab")
getcol = function(x,alph=1){
 chans = cramp(x)/255.0
 return(rgb(chans[1],chans[2],chans[3],alph))
}
y$col = sapply(y$probupper,getcol)

plot(y$porin,y$CI,pch=16,col=y$col)
