...
hiliteChannel = function(dat, hilite_ch = "NDUFB8", hilite_type = "theta (VDAC1)",hilite_default = "theta (VDAC1"){
 ...
 dhilite = dat[(as.character(dat$ch)==hilite_ch)&(dat$type==ifelse(hilite_type=="2Dmito",hilite_default,hilite_type)),]
...
updateDat = function(dat, dtype, dsubject, dhichan, cord = c(), clevel = 0.95,hilite_default="theta (VDAC1)", ctrl="Control"){
	...
	if(dhichan != " "){
	  hitype = ifelse(dtype=="2Dmito",hilite_default,dtype)
	  dvals$hcol = hiliteChannel(dat[(dat$patrep_id==dsubject)&(dat$type==hitype),], dhichan,hitype,hilite_default)
	...
	dvals$regression_diff = makeCond(dat, dsubject, clevel, ctrl)[paste(dvals$ch,dvals$cell_id)]
...
makeCond = function(dat,dsubject, clevel = 0.95, ctrl = "Control"){

	pimc = dat[(dat$patrep_id==dsubject)&(dat$type=="Mean intensity"),]
	cimc = dat[(dat$subject_group==ctrl)&(dat$type=="Mean intensity"),]
	pimc$regression_diff = "NODIFF"
	
	transform = log
	for(ch in cord[cord!=mitochan]){
      xctrl = transform(cimc$value[(as.character(cimc$ch)==mitochan)&(cimc$subject_group==ctrl)])
      yctrl = transform(cimc$value[(as.character(cimc$ch)==as.character(ch))&(cimc$subject_group==ctrl)])
...
overlaps = function(dat, dtype, dsubject, dhichan, row_condition, col_condition, cord = c(),counts = FALSE,mitochan="VDAC1"){

 if(dtype == paste("theta (",mitochan,")",sep="")){
   cats = "outlier_diff"
 }else if(dtype == paste("r (",mitochan,")",sep="")){
   cats = "outlier_diff"
 ...
 pat = data.frame(updateDat(dat, dtype, dsubject, dhichan, cord, hilite_default=paste("theta (",mitochan,")",sep="")))
